...
async function durableDefaults(projectRoot:string){
  let current=path.resolve(projectRoot);
  for(let i=0;i<8;i++){
    const file=path.join(current,'.windi','video-channel-defaults.json');
    if(await lstat(file).catch(()=>null))return {root:current,config:JSON.parse(await readFile(file,'utf8')) as {musicBackground?:{path?:string;volume?:number;loopToTimeline?:boolean}}};
...
function timedBeats(script:ScriptArtifact,captions:Caption[]){
  const total=Math.max(...captions.map(c=>c.endMs));let fallbackCursor=0;const weights=script.beats.map(beat=>Math.max(1,Array.from(beat.voiceOver).length));const weightTotal=weights.reduce((a,b)=>a+b,0);const starts=script.beats.map((beat,index)=>{
    if(index===0)return 0;
    const anchorWords=normalize(beat.spokenAnchor).split(' ').slice(0,4).filter(Boolean);
    const foundIndex=captions.findIndex((caption,cursor)=>cursor>=0&&caption.startMs>=fallbackCursor&&anchorWords.every((word,offset)=>normalize(captions[cursor+offset]?.text??'')===word));
    const found=foundIndex>=0?captions[foundIndex]:undefined;
    if(found){fallbackCursor=found.startMs;return found.startMs;}
...
export async function prepareRender(projectRoot:string){
  ...
  let presentation=JSON.parse(await readFile(path.join(projectRoot,'.windi','presentation.json'),'utf8').catch(error=>{if(error.code==='ENOENT')return 'null';throw error;}));
  if(!state.artifacts.imageManifest)throw new Error('IMAGE_MANIFEST_REQUIRED');const manifest=JSON.parse(await readFile(path.join(projectRoot,state.artifacts.imageManifest),'utf8')) as ImageManifest;let beats=timedBeats(script,captions);const exact=JSON.parse(await readFile(path.join(projectRoot,"windi/timing/beat-timing.json"),"utf8").catch(()=>"null"));if(exact?.scriptVersion===script.version&&path.resolve(projectRoot,state.artifacts.voice)===exact.audio&&exact.beats.length===beats.length)beats=beats.map((b,i)=>{if(exact.beats[i].id!==b.id)throw new Error("BEAT_TIMING_MISMATCH");return {...b,startMs:exact.beats[i].startMs,endMs:exact.beats[i].endMs};});for(const [index,beat] of beats.entries()){const job=manifest.jobs[index];if(!job)throw new Error(`IMAGE_JOB_MISSING_${index+1}`);let expected=path.join(projectRoot,job.output);if(!path.extname(expected)){for(const extension of ['.png','.jpg','.webp','.gif']){const candidate=`${expected}${extension}`;if(await lstat(candidate).catch(()=>null)){expected=candidate;break;}}}const original=await safeFile(projectRoot,expected);const source=job.provider==='flow'?(await cleanFlowImage(projectRoot,original)).output:original;const name=`scene-${String(index+1).padStart(2,'0')}${path.extname(source)}`;await copyFile(source,path.join(staging,name));(beat as typeof beat&{image:string}).image=`${relative}/${name}`;const media=presentation?.media?.[beat.id];if(media?.image){const mediaSource=await safeFile(projectRoot,path.join(projectRoot,media.image));const mediaName=`scene-${String(index+1).padStart(2,'0')}-custom${path.extname(mediaSource)}`;await copyFile(mediaSource,path.join(staging,mediaName));(beat as typeof beat&{image:string}).image=`${relative}/${mediaName}`;}if(media?.video){const mediaSource=await safeFile(projectRoot,path.join(projectRoot,media.video));const mediaName=`scene-${String(index+1).padStart(2,'0')}-custom${path.extname(mediaSource)}`;await copyFile(mediaSource,path.join(staging,mediaName));(beat as typeof beat&{video:string}).video=`${relative}/${mediaName}`;}if(media?.insetImage){const mediaSource=await safeFile(projectRoot,path.join(projectRoot,media.insetImage));const mediaName=`scene-${String(index+1).padStart(2,'0')}-custom-inset${path.extname(mediaSource)}`;await copyFile(mediaSource,path.join(staging,mediaName));media.insetImage=`${relative}/${mediaName}`;}}
  const durable=await durableDefaults(projectRoot);let music:string|undefined;let musicVolume:number|undefined;
  const musicSpec=durable?.config.musicBackground;if(durable&&musicSpec?.path){const musicSource=path.resolve(durable.root,musicSpec.path);if(await lstat(musicSource).catch(()=>null)){const musicName=`music${path.extname(musicSource)||'.mp3'}`;await copyFile(musicSource,path.join(staging,musicName));music=`${relative}/${musicName}`;musicVolume=Number.isFinite(musicSpec.volume)?musicSpec.volume:1;}}
...
export async function renderVideo(projectRoot:string){
  ...
  await run(prepared.renderer,['render','src/index.ts','WindiVideo',render,'--props',prepared.propsPath,'--codec','h264','--fps','30','--video-bitrate','15M','--audio-bitrate','192K','--pixel-format','yuv420p']);await run(prepared.renderer,['still','src/index.ts','WindiVideo',cover,'--props',prepared.propsPath,'--frame','0']);
