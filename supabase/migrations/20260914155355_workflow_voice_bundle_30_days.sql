begin;
update public.products set metadata=jsonb_set(metadata,'{voice_trial_duration_days}','30'::jsonb,true) where metadata->>'sku'='windi-video-workflow-v1';
update public.windi_voice_orders set duration_days=30 where gateway_id like 'workflow:%';
update public.windi_voice_periods p set ends_at=p.starts_at+interval '30 days' from public.windi_voice_orders o where o.id=p.order_id and o.gateway_id like 'workflow:%' and p.ends_at>now();
...
begin
 ...
 select * into target from public.orders where payment_code=p_code for update; if not found then return 'ignored'; end if;
 perform pg_advisory_xact_lock(871942001); if target.status='PAID' then return 'paid'; end if;
 ...
 for item in select i.id,i.product_id,p.type from public.order_items i join public.products p on p.id=i.product_id where i.order_id=target.id loop
  if item.type='VIDEO_KIT_LICENSE' then
   insert into public.product_entitlements(user_id,product_id,order_item_id,kind,voice_credits) values(target.user_id,item.product_id,item.id,'video_workflow_v1',0) on conflict(user_id,product_id) do update set updated_at=now() returning id into granted_entitlement;
   insert into public.windi_voice_orders(user_id,plan_id,amount_vnd,credits,clone_limit,duration_days,status,gateway_id,expires_at,paid_at) values(target.user_id,'trial',0,10000,1,30,'paid','workflow:'||granted_entitlement::text,now(),now()) on conflict(gateway_id) do update set gateway_id=excluded.gateway_id returning id into trial_order_id;
   insert into public.windi_voice_periods(user_id,order_id,plan_id,starts_at,ends_at,credits,clone_limit) values(target.user_id,trial_order_id,'trial',now(),now()+interval '30 days',10000,1) on conflict(order_id) do nothing returning id into trial_period_id;
