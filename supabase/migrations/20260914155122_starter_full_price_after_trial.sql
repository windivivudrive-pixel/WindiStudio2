...
-- The Clone trial bundled with Workflow is a standalone offer. Starter keeps
-- its published 69,000 VND price and is never reduced by the trial amount.
create or replace function public.windi_voice_order(p_user uuid,p_plan text)
returns public.windi_voice_orders
language plpgsql security invoker set search_path = '' as $$
declare plan public.windi_voice_plans; active_period public.windi_voice_periods; current_order public.windi_voice_orders; charge integer;
begin
 ...
 if plan.id='trial' and exists(select 1 from public.windi_voice_orders where user_id=p_user and status='paid' and plan_id in ('trial','starter','creator','studio')) then raise exception 'TRIAL_ALREADY_USED'; end if;
 ...
 if found and active_period.plan_id not in ('welcome','trial') then raise exception 'ACTIVE_PERIOD'; end if;
 update public.windi_voice_orders set status='expired' where user_id=p_user and status='pending' and expires_at<=now();
 select * into current_order from public.windi_voice_orders where user_id=p_user and status='pending' order by created_at desc limit 1;
 if found then
 ...
 values(p_user,plan.id,charge,plan.credits,plan.clone_limit,plan.duration_days) returning * into current_order;
