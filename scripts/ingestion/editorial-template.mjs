/**
 * Editorial and Easy Prompt Template for Non-Tech Creators
 * 
 * Rules:
 * 1. Web Tool Description: Concise, punchy hook, social proof, highlights features causing FOMO,
 *    clearly highlights benefits vs NOT using the tool. NO CTA at the end of web copy.
 * 2. Facebook Post: Rich copy, highlights, example, setup, limitations, AND CTA link with hashtags.
 * 3. Easy Prompt (Option 2): Action-first format starting with repo URL, numbered steps, safety rules,
 *    and clear input placeholders [Dán ... vào đây]. Never include patronizing non-tech boilerplate.
 */
...
export const CATEGORY_INPUTS_EN = {
...
/**
 * Format Option 2 Easy Prompt for any crawled tool or skill
 */
export function formatActionEasyPrompt({
  category = 'content',
  officialUrl,
}) {
  const cat = CATEGORY_GOALS[category] || CATEGORY_GOALS.content;
  const url = officialUrl || 'https://windistudio.app';
