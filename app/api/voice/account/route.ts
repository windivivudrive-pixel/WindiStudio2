...
export async function GET() {
 try {
  ...
  const [period,clones,jobs,orders]=await Promise.all([
    ...
    client.from('windi_voice_clones').select('id,provider_id,name,language,accent,status,created_at').eq('user_id',user.id).neq('status','deleted').order('created_at',{ascending:false}),
