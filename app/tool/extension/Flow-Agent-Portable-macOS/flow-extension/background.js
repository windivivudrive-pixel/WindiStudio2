...
const FLOW_TAB_URLS = ['https://flow.google.com/*', 'https://labs.google/fx/tools/flow*', 'https://labs.google/fx/*/tools/flow*'];
...
async function generateImageThroughFlowUi(tabId, requestId, body) {
  const requests = Array.isArray(body?.requests) ? body.requests : [];
  if (!requests.length) return { error: 'UI_FALLBACK_MISSING_REQUEST' };
  const inputCount = requests[0]?.imageInputs?.length || 0;
  const referenceCount = Math.max(0, Number(body?._flowAgentUiReferenceCount || 0));
  if (inputCount && referenceCount !== inputCount) return { error: 'UI_FALLBACK_REFERENCE_NOT_RECENT' };
  const prompt = requests[0]?.structuredPrompt?.parts?.map((part) => part?.text || '').join('\n').trim();
  if (!prompt) return { error: 'UI_FALLBACK_MISSING_PROMPT' };
  const response = await chrome.tabs.sendMessage(tabId, {
    type: 'UI_GENERATE_IMAGE', requestId, prompt,
    aspect: requests[0]?.imageAspectRatio || 'IMAGE_ASPECT_RATIO_SQUARE',
    count: Math.min(4, requests.length),
    referenceCount,
  });
  if (!response?.images?.length) return { error: response?.error || 'UI_FALLBACK_NO_IMAGE' };
  return {
    status: 200,
    data: {
      media: response.images.map((image) => ({
        name: crypto.randomUUID(),
        image: { generatedImage: { imageUri: image } },
      })),
    },
  };
}
...
async function handleApiRequest(msg) {
  ...
  try {
    ...
    if (captchaAction) {
      ...
      if (!captchaToken) {
        // flow.google.com no longer exposes the legacy grecaptcha object. Use
        // the signed-in Flow UI for text-to-image jobs and let the page handle
        // its normal verification flow. This never solves or reuses CAPTCHA.
        const captchaError = captchaResult?.error || 'CAPTCHA_FAILED';
        if (captchaAction === 'IMAGE_GENERATION' && captchaError.includes('grecaptcha not available')) {
          const tab = await getOrOpenFlowTab();
          const uiResult = tab ? await generateImageThroughFlowUi(tab.id, id, body) : { error: 'NO_FLOW_TAB' };
          if (uiResult?.status === 200) {
            sendToAgent({ id, status: 200, data: uiResult.data });
            metrics.successCount++;
            chrome.storage.local.set({ metrics });
            updateRequestLog(logId, { status: 'success', responseSummary: 'Generated through Flow UI' });
            setState('idle');
            return;
          }
          sendToAgent({ id, status: 409, error: uiResult?.error || 'UI_FALLBACK_FAILED' });
          metrics.failedCount++;
          metrics.lastError = uiResult?.error || 'UI_FALLBACK_FAILED';
          chrome.storage.local.set({ metrics });
          updateRequestLog(logId, { status: 'failed', error: metrics.lastError });
          setState('idle');
          return;
        }
    ...
    if (captchaToken && finalBody) {
      ...
      delete finalBody._flowAgentUiReferenceCount;
