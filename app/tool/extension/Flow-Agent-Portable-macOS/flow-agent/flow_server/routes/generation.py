...
async def _generate_image(req: ImageGenerationRequest, x_client_id: Optional[str] = None):
    ...
    encoded_refs = list(req.reference_images_base64 or [])
    if req.image_base64:
        encoded_refs.insert(0, req.image_base64)
    if len(encoded_refs) + len(ref_media_ids or []) > 10:
        raise HTTPException(status_code=400, detail="Image generation accepts at most 10 references.")

    if encoded_refs:
        from flow_engine.generators.i2v import upload_image
        ref_media_ids = list(ref_media_ids or [])
        for encoded_ref in encoded_refs:
            media_bytes, mime_type = _decode_base64_media(encoded_ref)
            if not mime_type.startswith("image/"):
                raise HTTPException(status_code=400, detail=f"I2I reference must be an image; received {mime_type}.")
            extension = extension_for_media(media_bytes, declared_mime=mime_type)
            temp_img_path = os.path.join(OUTPUT_DIR, f"i2i_upload_{int(time.time())}_{uuid.uuid4().hex[:6]}{extension}")
            try:
                with open(temp_img_path, "wb") as f:
                    f.write(media_bytes)
                media_id = await upload_image(active_bridge, temp_img_path, project_id, force_upload=True)
                if not media_id:
                    raise RuntimeError("Flow returned no media ID")
                ref_media_ids.append(media_id)
                ui_reference_count += 1
        ...
        for chunk_size in chunks:
            tasks.append(
                generate_image(
                    ...
                    ui_reference_count=ui_reference_count,
                )
            )
