...
def cmd_image(argv):
    ...
    if len(local_refs) > 10:
        parser.error("flow image accepts at most 10 local reference files")
    if local_refs:
        payload["reference_images_base64"] = [_file_data_uri(path) for path in local_refs]
    if media_ids:
